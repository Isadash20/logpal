...
/**
 * My favourites — the foods you starred.
 *
 * The star on a food already worked and already wrote to `favoriteFoodIds`;
 * what it did not have was anywhere to go. Starring something and then not
 * being able to find it is worse than no star at all, so this is the place it
 * points to, sitting alongside My meals, My recipes and My foods where anyone
 * looking for their own things would look.
 *
 * Most recently starred first: the reason to keep a list like this is the thing
 * you are eating at the moment, not the thing you starred in March.
 */
export function MyFavorites() {
  const { pop, push, data, resolveFood, toggleFavorite, date } = useApp()

  const foods = data.favoriteFoodIds
    .map((id) => resolveFood(id))
    .filter((f): f is Food => !!f)

  return (
    <>
      <TopBar title="My Favourites" onBack={pop} solid />
      <div className="scroll">
        {foods.length === 0 ? (
          <Empty title="Nothing starred yet">
            Star a food from its page — the one you scan every morning, the brand you
            always buy — and it waits here instead of being searched for again.
          </Empty>
        ) : (
            ...
            {foods.map((f) => (
              <Row
                ...
                right={
                  ...
                  >
                    <Icon name="star-filled" size={19} />
                  </button>
                }
                onClick={() => push({ name: 'foodDetail', food: f, date })}
              />
            ))}
          </div>
        )}
